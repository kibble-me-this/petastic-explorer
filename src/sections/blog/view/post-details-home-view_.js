...
// api
import { useGetPost, useGetLatestPosts } from 'src/api/blog_';
// components
...
export default function PostDetailsHomeView({ title }) {
  console.log("PostDetailsHomeView: title", title);

  const { post, postError, postLoading } = useGetPost(title);

  console.log("PostDetailsHomeView post: ", post);
  ...
  const renderPost = post && (
    <>
      {  console.log("PostDetailsHomeView in renderPost: ", post) }

      <PostDetailsHero
        title={post.title}
        // author={post.author}
        coverUrl={post.hero_image_url}
